...
namespace MapTool
{
    public class CreateNewLevelWindow
    {
        ...
        private void CreateLevel()
        {
            ...
            var gridData = ScriptableObject.CreateInstance<MapDataSO>();
        ...
        private void SetUpMapBaker(Scene scene, MapDataSO mapData)
        {
            //Find or create MapEditor game object
            MapBaker mapBaker = GameObject.FindAnyObjectByType<MapBaker>();
            GameObject mapBakerGO = null;

            if (mapBaker == null)
            {
                mapBakerGO = new GameObject("MapBaker");
                mapBaker = mapBakerGO.AddComponent<MapBaker>();
            }
            else
            {
            ...
            GUID[] terrainGUIDs = AssetDatabase.FindAssetGUIDs($"t:{nameof(TerrainTypeSO).ToString()}", new[] { settings.terrainTypePath });
            List<TerrainTypeSO> terrainTypes = new List<TerrainTypeSO>();

            Debug.Log("Terrain GUIDS: " + terrainGUIDs.Length + " " + settings.terrainTypePath);
